...
Edit `search.sources` and `search.categories` in `config.toml` for the feeds and professions you need. By default, the bot searches **development/testing** and **IT systems, networks, and support** using each source's structured professional categories, not words in a job description. Set `categories = []` to return to the older `search.queries` text mode. The included starter configuration searches HH in Russia plus worldwide remote listings. HH's `area_ids` and `experience_ids` apply only to HH. Every international listing shows its stated location scope. Check whether the employer can hire from your country before applying: remote does not mean worldwide.
