...
def load_templates(path: Path) -> tuple[ReplyTemplate, ...]:
    ...
    if not isinstance(raw, dict) or set(raw) != {item.key for item in DEFAULT_TEMPLATES}:
    ...
    for default in DEFAULT_TEMPLATES:
        ...
        result.append(replace(default, body=body, keywords=tuple(keywords)))
    return tuple(result)


def save_template_field(path: Path, key: str, field: str, text: str) -> None:
    if field not in {"body", "keywords"} or key not in {item.key for item in DEFAULT_TEMPLATES}:
    ...
    temporary.write_text(
        json.dumps(
            {item.key: {"keywords": item.keywords, "body": item.body} for item in templates},
            ensure_ascii=False,
            indent=2,
        ),
        encoding="utf-8",
...
def select_template(vacancy: Vacancy, templates: tuple[ReplyTemplate, ...]) -> ReplyTemplate:
    ...
    for template in templates:
        if any(word.casefold().replace("ё", "е") in title for word in template.keywords):
            return template
...
def render_reply(profile: CandidateProfile, vacancy: Vacancy, template: ReplyTemplate) -> str:
    ...
    for line in template.body.splitlines():
        ...
        if (
            line.strip() in {"{skills_line}", "{resume_line}", "{portfolio_line}"}
